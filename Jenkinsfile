            ...
            steps {
                nodejs(nodeJSInstallationName: 'nodejs') {
                    withCredentials([usernamePassword(credentialsId: 'pinglink-deployer', passwordVariable: 'PASSWORD', usernameVariable: 'USERNAME')]) {
                        ...
                        sh "sshpass -p '${PASSWORD}' ssh -o StrictHostKeyChecking=no ${USERNAME}@pinglink-ui.keyssoft.xyz which node && node -v && ls -ltr /usr/bin  && cd pinglink-server && npm install"
